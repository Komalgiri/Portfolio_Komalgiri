...
const ExperienceCard = ({ exp, align }: { exp: ExperienceItem; align: 'left' | 'right' }) => (
    <div className={`space-y-4 ${align === 'right' ? 'md:text-right' : ''}`}>
        <div className={`flex flex-col gap-1.5 ${align === 'right' ? 'md:items-end' : 'md:items-start'}`}>
            ...
            <h3 className="text-3xl font-black text-theme-text">{exp.role}</h3>
            <p className="text-base font-bold text-theme-muted">
                {exp.company} · {exp.employmentType}
            </p>
            <div
                className={`flex items-center gap-2 text-base text-theme-muted ${
        ...
        <p className="text-base md:text-lg leading-relaxed text-theme-muted">{exp.summary}</p>
        ...
        {exp.metrics && exp.metrics.length > 0 && (
            <div className={`flex flex-wrap gap-2 ${align === 'right' ? 'md:justify-end' : ''}`}>
                {exp.metrics.map((m) => (
                    <div
                        key={m.label}
                        className="rounded-xl border border-purple-500/20 bg-purple-500/5 px-4 py-3 text-center"
                    >
                        <p className="text-xl md:text-2xl font-black text-purple-400">{m.value}</p>
                        <p className="text-xs font-medium uppercase tracking-wider text-theme-muted">{m.label}</p>
                    </div>
                ))}
            </div>
        )}
        ...
        <ul className={`space-y-2 ${align === 'right' ? 'md:text-right' : ''}`}>
            {exp.highlights.map((point) => (
                <li
                    key={point}
                    className={`flex gap-2 text-base md:text-lg leading-relaxed text-theme-muted ${
                        align === 'right' ? 'md:flex-row-reverse' : ''
                    }`}
                >
